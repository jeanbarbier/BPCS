lambda_dantzig = 0.00516; N = 256; n_iter = 500; alpha = 0.5; rho_corrupt = 0.1; tt = 1; M = ceil(N ./ (1 - alpha) ); k = floor(rho_corrupt .* M);
disp(rho_corrupt)
disp(alpha)
pause
...
for t = 1 : n_iter
    ...
    Y = Y_null; F = G_null';
    % Dantzig selector
    err_L1_3 = l1dantzig_pd(F' * Y, F, [], Y, lambda_dantzig, 1e-3,1000);
    % regression step
...
%% save
save(['L1_LambdaDant',num2str(lambda_dantzig),'_a',num2str(alpha),'_rho',num2str(rho_corrupt),'_eps6','.mat'],'results_L1');