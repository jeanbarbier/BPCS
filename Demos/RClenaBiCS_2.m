...
for ll = 1 : max(size(Phantom) ) / sqrt(N)
    for cc = 1 : max(size(Phantom) ) / sqrt(N)
        
        % patch of Phantom
        S = double(Phantom((ll - 1) * sqrt(N) + 1: ll * sqrt(N), (cc - 1) * sqrt(N) + 1 : cc * sqrt(N) ) );
...
if (saveRes == 1); save(name,'I_id','I_CS','I_L1','I_naive','results_'); end